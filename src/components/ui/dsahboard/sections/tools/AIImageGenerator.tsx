  ...
  const handleGenerate = async () => {
    ...
    try {
      const imageUrl = await generateWithPollinations(
        concatPromptWithModel(pollinationsModel, prompt),
        width,
        height
      );
  ...
  const handleImprovePrompt = async () => {
    ...
    if (isImproved) {
      ...
      try {
        ...
        setOriginalPrompt(prompt); // Save original before improving

        const response = await imageGenService.improvePrompt(prompt);
              ...
              <div className="bg-white rounded-2xl shadow-lg border border-indigo-100 p-5 transition-all duration-300 hover:shadow-xl">
                ...
                <textarea
                  ...
                  onChange={(e) => {
                    ...
                    // Reset improved state if user manually edits
                    if (isImproved) {
                      setIsImproved(false);
                    }
                  }}
