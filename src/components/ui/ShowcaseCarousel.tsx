...
export const ShowcaseCarousel: React.FC<ShowcaseCarouselProps> = ({
  ...
  const handleSlideClick = (item: any, clickedIndex: number) => {
    if (!swiperRef.current) return;

    const swiper = swiperRef.current;
    const activeIndex = swiper.realIndex; // Get the actual active slide index (accounting for loop)

    // Check if the clicked slide is the center/active one
    if (clickedIndex === activeIndex) {
      // Slide is already centered - open modal
      setSelectedItem(item);
      setIsModalOpen(true);
    } else {
      // Slide is not centered - scroll to center it
  ...
  const getTemplateName = (item: any) => {
    if (type === "render" && item.templateId) {
      return templatesWithTheirIds?.[String(item.templateId)] || "Unknown Template";
    }
    return undefined;
  };
  ...
  const getFormattedDate = (item: any) => {
    if (type === "render") {
      return formatDateSafe(item.renderedAt);
    } else if (type === "project") {
      return formatDateSafe(item.createdAt);
    }
    return undefined;
  };
  ...
  return (
    <>
      ...
      <ViewMediaModal
        ...
        item={selectedItem}
        itemType={type}
        templateName={selectedItem ? getTemplateName(selectedItem) : undefined}
        formattedDate={selectedItem ? getFormattedDate(selectedItem) : undefined}
      />
    </>
  );
};