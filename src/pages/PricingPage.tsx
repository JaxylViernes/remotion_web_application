...
const PricingPage: React.FC = () => {
  ...
  const faqs = [
    ...
    },
    {
      ...
      answer: 'Credits reset each month, so make sure to use them before your billing cycle ends.',
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-white font-[Inter,-apple-system,BlinkMacSystemFont,sans-serif] overflow-x-hidden">
      ...
      {/* Navigation */}
      <nav className="fixed top-0 left-0 right-0 z-[1000] bg-white/95 backdrop-blur-[20px] border-b border-gray-100 shadow-sm transition-all duration-300">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            >
              <button className="flex items-center gap-1.5 text-gray-600 text-sm font-medium bg-transparent border-none cursor-pointer py-2 transition-colors hover:text-gray-900 group">
                Features
                <svg className={`transition-transform duration-200 ${featuresDropdownOpen ? 'rotate-180' : ''}`} width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M6 9l6 6 6-6" />
                </svg>
              </button>
                ...
                <div className="absolute top-full left-1/2 -translate-x-1/2 bg-white rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.1)] p-2 min-w-[200px] z-[1000] border border-gray-100">
                  {featureItems.map((item, idx) => (
                    <Link
                      key={item.name}
                      to={item.path}
                      className={`block w-full py-2.5 px-3 text-gray-600 no-underline text-sm font-medium rounded-lg transition-all hover:bg-gray-50 hover:text-gray-900 opacity-0-initial animate-fade-in-up stagger-${idx + 1}`}
            ...
            >
              <button className="flex items-center gap-1.5 text-gray-600 text-sm font-medium bg-transparent border-none cursor-pointer py-2 transition-colors hover:text-gray-900 group">
                Templates
                <svg className={`transition-transform duration-200 ${templatesDropdownOpen ? 'rotate-180' : ''}`} width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  <path d="M6 9l6 6 6-6" />
                </svg>
              </button>
            ...
            <Link to="/pricing" className="text-gray-900 no-underline text-sm font-semibold" onClick={() => window.scrollTo(0, 0)}>Pricing</Link>
          </div>
          <Link to="/login" className="hidden md:inline-block bg-gray-900 text-white py-2.5 px-6 rounded-full font-semibold text-sm no-underline transition-all duration-300 hover:bg-violet-600 hover:scale-105 hover:shadow-lg" onClick={() => window.scrollTo(0, 0)}>Login</Link>
        </div>
        ...
        <div className={`md:hidden overflow-hidden transition-all duration-300 ${mobileMenuOpen ? 'max-h-[500px]' : 'max-h-0'}`}>
          <div className="px-4 py-4 bg-white border-t border-gray-100 flex flex-col gap-2">
            ...
            <div className="pt-2 border-t border-gray-100">
              <Link to="/pricing" className="block py-2 px-3 text-gray-900 no-underline text-sm font-medium rounded-lg" onClick={() => { setMobileMenuOpen(false); window.scrollTo(0, 0); }}>Pricing</Link>
              <Link to="/login" className="block mt-2 bg-gray-900 text-white py-3 px-4 rounded-full font-semibold text-sm no-underline text-center" onClick={() => { setMobileMenuOpen(false); window.scrollTo(0, 0); }}>Login</Link>
            </div>
          </div>
        </div>
      </nav>
      ...
      <section className="pt-20 md:pt-24 px-4 md:px-8 pb-6">
        <div
          ref={heroAnim.ref}
          className={`max-w-[1150px] mx-auto transition-all duration-700 ${heroAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
        >
          {/* Main Pricing Card */}
          <div className="bg-purple-200 rounded-[32px] p-6 md:p-8 relative overflow-hidden">
            ...
            <div className="relative z-10 text-center">
              {/* Title */}
              <h1 className={`text-4xl md:text-5xl lg:text-6xl font-extrabold mb-1 leading-tight transition-all duration-500 ${heroAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`} style={{ transitionDelay: '0.1s' }}>
                <span className="bg-gradient-to-r from-purple-600 via-pink-500 to-purple-600 bg-clip-text text-transparent">One Plan</span>
              </h1>
              <h2 className={`text-3xl md:text-4xl lg:text-5xl font-extrabold text-gray-900 mb-3 leading-tight transition-all duration-500 ${heroAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`} style={{ transitionDelay: '0.2s' }}>
                Unlimited potential
              </h2>
              
              {/* Subtitle */}
              <p className={`text-gray-700 font-medium text-sm md:text-base mb-4 transition-all duration-500 ${heroAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`} style={{ transitionDelay: '0.3s' }}>
                No hidden fees, no complicated tires
              </p>
              
              {/* Price */}
              <div className={`mb-4 transition-all duration-500 ${heroAnim.isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`} style={{ transitionDelay: '0.4s' }}>
                <div className="flex items-baseline justify-center gap-1">
                  <span className="text-xl font-bold text-gray-400">$</span>
                  <span className="text-6xl md:text-7xl font-extrabold text-gray-900 tracking-tight">19.99</span>
                  <span className="text-lg font-medium text-gray-400">/mo</span>
                </div>
              </div>
              
              {/* Description */}
              <p className={`text-gray-500 text-sm mb-6 transition-all duration-500 ${heroAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`} style={{ transitionDelay: '0.5s' }}>
      ...
      <section className="px-4 md:px-8 pb-40">
        <div className={`flex flex-col items-center gap-2 transition-all duration-500 ${heroAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`} style={{ transitionDelay: '0.8s' }}>
          <svg className="animate-bounce text-gray-400" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M6 9l6 6 6-6" />
          </svg>
      ...
      <section className="px-4 md:px-8 pb-20">
        <div 
          ...
          className={`max-w-[700px] mx-auto text-center transition-all duration-700 ${unlimitedAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
        >
          {/* Title with infinity icon */}
          <div className={`flex items-center justify-center gap-3 mb-10 transition-all duration-500 ${unlimitedAnim.isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-90'}`}>
            <svg width="36" height="36" viewBox="0 0 24 24" fill="none" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <defs>
              ...
              <path stroke="url(#infinityGradient)" d="M18.178 8c5.096 0 5.096 8 0 8-5.095 0-7.133-8-12.739-8-4.585 0-4.585 8 0 8 5.606 0 7.644-8 12.74-8z" />
            </svg>
            <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Unlimited Access</h2>
          </div>
          ...
          <div className="flex flex-wrap justify-center gap-4 md:gap-6">
            {unlimitedItems.map((item, idx) => (
              <div
                key={item.name}
                className={`transition-all duration-500 ${unlimitedAnim.isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-75'}`}
              ...
              >
                <div className="w-24 h-24 md:w-28 md:h-28 rounded-full bg-gradient-to-br from-purple-600 via-purple-500 to-pink-500 flex items-center justify-center shadow-lg shadow-purple-500/30 hover:shadow-xl hover:shadow-purple-500/40 hover:scale-110 transition-all duration-300 cursor-default">
                  <span className="text-white font-bold text-sm md:text-base text-center px-2">{item.name}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="px-4 md:px-8 pb-35">
        <div 
          ...
          className={`max-w-[1150px] mx-auto transition-all duration-700 ${featuresAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
        >
          <div className="bg-purple-200 rounded-[32px] p-8 md:p-10 relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col items-center">
              <h3 className={`text-xl md:text-2xl font-bold text-gray-900 text-center mb-8 transition-all duration-500 ${featuresAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
              ...
              <ul className="space-y-4 inline-block">
                {features.map((feature, idx) => (
                  <li 
                    key={idx} 
                    className={`flex items-center gap-3 transition-all duration-500 ${featuresAnim.isVisible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-5'}`}
        ...
        >
          <div className="grid grid-cols-3 gap-4 md:gap-8">
            {stats.map((stat, idx) => (
              <div 
                key={idx} 
                className={`text-center transition-all duration-700 ${statsAnim.isVisible ? 'opacity-100 scale-100' : 'opacity-0 scale-75'}`}
      ...
      <section className="px-4 md:px-8 pb-40">
        ...
        >
          <h2 className={`text-2xl md:text-3xl font-bold text-gray-900 text-center mb-12 transition-all duration-500 ${faqAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}>
          ...
          <div className="space-y-22">
            {faqs.map((faq, idx) => (
              <div 
                key={idx}
                className={`transition-all duration-500 ${faqAnim.isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-5'}`}
        ...
        <div className="max-w-[700px] mx-auto">
          <div className="bg-gradient-to-r from-purple-600 to-pink-500 rounded-[32px] p-8 md:p-12 text-center">
            ...
            <p className="text-white/80 mb-8 max-w-[400px] mx-auto">
              Join thousands of creators making engaging content with ViralMotion.
            </p>
            <Link
              to="/signup"
